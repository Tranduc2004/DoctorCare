...
// API functions
export const createMedicalRecordFromAppointment = async (appointmentId: string, doctorId: string): Promise<MedicalRecord> => {
  const response = await axios.post(`/doctor/medical-records/from-appointment/${appointmentId}`, {
    doctorId
  });
  return response.data;
};

export const getMedicalRecordByAppointment = async (appointmentId: string): Promise<MedicalRecord> => {
  const response = await axios.get(`/doctor/medical-records/by-appointment/${appointmentId}`);
  return response.data;
};

export const updateMedicalRecord = async (recordId: string, data: Partial<MedicalRecord>, doctorId: string): Promise<MedicalRecord> => {
  const response = await axios.put(`/doctor/medical-records/${recordId}`, { ...data, doctorId });
  return response.data;
};

export const getDoctorMedicalRecords = async (doctorId: string): Promise<MedicalRecord[]> => {
  const response = await axios.get(`/doctor/medical-records?doctorId=${doctorId}`);
  return response.data;
};

export const getMedicalRecordDetail = async (recordId: string, doctorId: string): Promise<MedicalRecord> => {
  const response = await axios.get(`/doctor/medical-records/${recordId}?doctorId=${doctorId}`);
...
export const getPatientsWithCompletedAppointments = async (doctorId: string): Promise<PatientWithCompletedAppointment[]> => {
  const response = await axios.get(`/doctor/medical-records/patients/completed?doctorId=${doctorId}`);
  return response.data.data;
};