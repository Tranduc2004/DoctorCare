...
const MedicalRecordForm: React.FC<MedicalRecordFormProps> = ({
  ...
  const addMedication = () => {
    const newMedication = {
      ...
      frequency: getValue("medications.frequency") || 1,
      duration: getValue("medications.duration") || 1,
      quantity: getValue("medications.quantity") || 1,
    ...
    try {
      ...
      if (data && data._id) {
        ...
        if (data.prescription?.medications) {
          const loadedMedications = data.prescription.medications.map((med, index) => ({
            id: Date.now() + index,
            drugName: med.name,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedRecord = await updateMedicalRecord(record._id, recordToSave, user._id);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const updatedRecord = await updateMedicalRecord(record._id, draftRecord, user._id);
      setRecord(updatedRecord);
      onSave?.(updatedRecord);
  ...
  const handleIssuePrescription = async () => {
    ...
    try {
      ...
      const updatedRecord = await updateMedicalRecord(record._id, prescriptionRecord, user._id);
      setRecord(updatedRecord);
      onSave?.(updatedRecord);
...
Danh sách thuốc:
${record.treatment?.medicationsList?.map((med: any, index: number) => 
  `${index + 1}. ${med.drugName} ${med.strength} ${med.form} - ${med.dosage} - ${med.frequency} - ${med.duration}`
).join('\n') || "Không có thuốc"}
...
Bác sĩ: ${user?.fullName || "N/A"}
      ...
      link.href = url;
      link.download = `tom-tat-ho-so-${record.patientInfo?.fullName || 'benh-nhan'}-${new Date().toISOString().split('T')[0]}.txt`;
  ...
  const handleCompleteRecord = async () => {
    ...
    if (!record.diagnosis?.primaryDiagnosis) {
      validationErrors.push("Chẩn đoán chính");
    }
    if (!record.treatment) {
      validationErrors.push("Kế hoạch điều trị");
    }
    if (!record.symptoms?.chiefComplaint) {
      validationErrors.push("Triệu chứng chính");
    }

    if (validationErrors.length > 0) {
      alert(`Vui lòng hoàn thành các thông tin sau trước khi hoàn thành hồ sơ:\n- ${validationErrors.join('\n- ')}`);
      return;
    }
    ...
    try {
      ...
      const updatedRecord = await updateMedicalRecord(record._id, completedRecord, user._id);
      setRecord(updatedRecord);
      onSave?.(updatedRecord);
  ...
  const updateRecord = (path: string, value: any) => {
    if (!record) return;
  ...
  const calculateBMI = () => {
    const weight = getValue("quickScreening.weight");
    const height = getValue("quickScreening.height");
  ...
  useEffect(() => {
    calculateBMI();
  }, [getValue("quickScreening.weight"), getValue("quickScreening.height")]);
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-teal-50">
          ...
          <div className="flex items-center gap-2">
            <div className="text-sm text-gray-600">
              Trạng thái: <span className="font-medium">{record?.status || "draft"}</span>
            </div>
            <button
              onClick={handleSave}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              {activeTab === "screening" && (
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        id="identityVerified"
                        checked={getValue("quickScreening.identityVerified") || false}
                        onChange={(e) =>
                          updateRecord("quickScreening.identityVerified", e.target.checked)
                        }
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                      <label htmlFor="identityVerified" className="text-sm font-medium text-gray-700">
                  ...
                  {/* Vitals */}
                  <div>
                    <h4 className="text-md font-medium text-gray-900 mb-3">Vitals</h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("quickScreening.temperature", parseFloat(e.target.value))
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("quickScreening.bloodPressure", e.target.value)
                          }
                          placeholder="120/80"
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("quickScreening.heartRate", parseInt(e.target.value))
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("quickScreening.oxygenSaturation", parseInt(e.target.value))
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("quickScreening.weight", parseFloat(e.target.value))
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("quickScreening.height", parseInt(e.target.value))
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          ...
                          />
                          {getValue("quickScreening.bmi") && (
                            <div className={`text-xs font-medium ${getBMIClassification(getValue("quickScreening.bmi")).color}`}>
                              {getBMIClassification(getValue("quickScreening.bmi")).text}
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  {/* Allergies */}
                  <div>
                    <h4 className="text-md font-medium text-gray-900 mb-3">Dị ứng</h4>
                    <div className="space-y-3">
                      <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2">
                          <input
                            type="radio"
                            name="allergies"
                            checked={!getValue("quickScreening.allergies.hasAllergies")}
                            onChange={() =>
                              updateRecord("quickScreening.allergies.hasAllergies", false)
                            }
                        ...
                        <label className="flex items-center gap-2">
                          <input
                            ...
                            checked={getValue("quickScreening.allergies.hasAllergies")}
                            onChange={() =>
                              updateRecord("quickScreening.allergies.hasAllergies", true)
                            }
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500"
                          />
                      ...
                      {getValue("quickScreening.allergies.hasAllergies") && (
                        <div>
                          ...
                          <textarea
                            value={getValue("quickScreening.allergies.allergyList")}
                            onChange={(e) =>
                              updateRecord("quickScreening.allergies.allergyList", e.target.value)
                            }
                            rows={2}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateRecord("quickScreening.currentMedications", e.target.value)
                      }
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                  ...
                  {getValue("patientInfo.gender") === "female" && (
                    <div>
                      <h4 className="text-md font-medium text-gray-900 mb-3">Mang thai/cho bú</h4>
                      <div className="space-y-3">
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            id="isPregnant"
                            checked={getValue("quickScreening.pregnancyStatus.isPregnant") || false}
                            onChange={(e) =>
                              updateRecord("quickScreening.pregnancyStatus.isPregnant", e.target.checked)
                            }
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                          />
                        ...
                        {getValue("quickScreening.pregnancyStatus.isPregnant") && (
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                              Tuần thai
                            </label>
                            <input
                              type="number"
                              value={getValue("quickScreening.pregnancyStatus.gestationalWeeks")}
                              onChange={(e) =>
                                updateRecord("quickScreening.pregnancyStatus.gestationalWeeks", parseInt(e.target.value))
                              }
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            />
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            id="isBreastfeeding"
                            checked={getValue("quickScreening.pregnancyStatus.isBreastfeeding") || false}
                            onChange={(e) =>
                              updateRecord("quickScreening.pregnancyStatus.isBreastfeeding", e.target.checked)
                            }
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                          />
                          <label htmlFor="isBreastfeeding" className="text-sm text-gray-700">
                            Cho bú
                          </label>
                        </div>
                      </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateRecord("quickScreening.receptionNotes", e.target.value)
                      }
                      rows={2}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateRecord("symptomDetails.location", e.target.value)
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    ...
                    <div className="col-span-2">
                      ...
                      <div className="space-y-3">
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("symptomDetails.painScale", parseInt(e.target.value))
                          }
                          className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                          style={{
                            background: `linear-gradient(to right, #10b981 0%, #f59e0b ${(getValue("symptomDetails.painScale") || 0) * 5}%, #ef4444 50%, #dc2626 100%)`
                          }}
                        />
                        ...
                        <div className="text-center">
                          <span className={`text-lg font-bold ${
                            (getValue("symptomDetails.painScale") || 0) <= 3 ? 'text-green-600' :
                            (getValue("symptomDetails.painScale") || 0) <= 6 ? 'text-yellow-600' :
                            'text-red-600'
                          }`}>
                            {getValue("symptomDetails.painScale") || 0}/10
                          </span>
                          <div className="text-sm text-gray-600 mt-1">
                            {(getValue("symptomDetails.painScale") || 0) === 0 && "Không đau"}
                            {(getValue("symptomDetails.painScale") || 0) >= 1 && (getValue("symptomDetails.painScale") || 0) <= 3 && "Đau nhẹ"}
                            {(getValue("symptomDetails.painScale") || 0) >= 4 && (getValue("symptomDetails.painScale") || 0) <= 6 && "Đau vừa"}
                            {(getValue("symptomDetails.painScale") || 0) >= 7 && (getValue("symptomDetails.painScale") || 0) <= 8 && "Đau nặng"}
                            {(getValue("symptomDetails.painScale") || 0) >= 9 && "Đau rất nặng"}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateRecord("symptomDetails.duration", e.target.value)
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateRecord("symptomDetails.aggravatingFactors", e.target.value)
                        }
                        rows={3}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateRecord("symptomDetails.relievingFactors", e.target.value)
                        }
                        rows={3}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateRecord("symptomDetails.associatedSymptoms", e.target.value)
                      }
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                  ...
                  {/* Attachments */}
                  <div>
                    <h4 className="text-md font-medium text-gray-900 mb-3">Ảnh/video (tùy chọn)</h4>
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                      <Camera className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                      <p className="text-sm text-gray-600">Kéo thả hoặc click để tải lên ảnh/video</p>
                      <input
                        type="file"
                        multiple
                        accept="image/*,video/*"
              ...
              {activeTab === "history" && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => updateRecord("medicalHistory.syncFromPrevious", true)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateRecord("medicalHistory.pastMedicalHistory", e.target.value)
                        }
                        rows={4}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateRecord("medicalHistory.surgicalHistory", e.target.value)
                        }
                        rows={4}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateRecord("medicalHistory.familyHistory", e.target.value)
                        }
                        rows={4}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <input
                            type="text"
                            value={getValue("medicalHistory.socialHistory.smoking")}
                            onChange={(e) =>
                              updateRecord("medicalHistory.socialHistory.smoking", e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="text"
                            value={getValue("medicalHistory.socialHistory.alcohol")}
                            onChange={(e) =>
                              updateRecord("medicalHistory.socialHistory.alcohol", e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="text"
                            value={getValue("medicalHistory.socialHistory.occupation")}
                            onChange={(e) =>
                              updateRecord("medicalHistory.socialHistory.occupation", e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="text"
                            value={getValue("medicalHistory.socialHistory.other")}
                            onChange={(e) =>
                              updateRecord("medicalHistory.socialHistory.other", e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                        </div>
              ...
              {activeTab === "examination" && (
                <div className="space-y-6">
                  ...
                  {/* General Examination */}
                  <div>
                    <h4 className="text-md font-medium text-gray-900 mb-3">Toàn thân</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Tỉnh táo, thể trạng
                        ...
                        <input
                          type="text"
                          value={getValue("clinicalExamination.generalAppearance")}
                          onChange={(e) =>
                            updateRecord("clinicalExamination.generalAppearance", e.target.value)
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.consciousness", e.target.value)
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.nutrition", e.target.value)
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.skinMucosa", e.target.value)
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                  ...
                  {/* System Examination */}
                  <div>
                    <h4 className="text-md font-medium text-gray-900 mb-3">Theo hệ cơ quan</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Tim mạch
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.cardiovascular", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.respiratory", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          value={getValue("clinicalExamination.gastrointestinal")}
                          onChange={(e) =>
                            updateRecord("clinicalExamination.gastrointestinal", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.neurological", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          value={getValue("clinicalExamination.musculoskeletal")}
                          onChange={(e) =>
                            updateRecord("clinicalExamination.musculoskeletal", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.genitourinary", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateRecord("clinicalExamination.endocrine", e.target.value)
                          }
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateRecord("clinicalExamination.examinationNotes", e.target.value)
                      }
                      rows={4}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
              ...
              {activeTab === "paraclinical" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Laboratory Tests */}
                    <div>
                      <h4 className="text-md font-medium text-gray-900 mb-3">Xét nghiệm</h4>
                      <div className="space-y-3">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
                          ...
                          <textarea
                            value={Array.isArray(getValue("paraclinicalIndications.laboratoryTests.tests")) ? getValue("paraclinicalIndications.laboratoryTests.tests").join('\n') : ''}
                            onChange={(e) =>
                              updateRecord("paraclinicalIndications.laboratoryTests.tests", e.target.value.split('\n').filter(t => t.trim()))
                            }
                            rows={4}
                        ...
                        <div>
                          ...
                          <textarea
                            value={getValue("paraclinicalIndications.laboratoryTests.notes")}
                            onChange={(e) =>
                              updateRecord("paraclinicalIndications.laboratoryTests.notes", e.target.value)
                            }
                            rows={2}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                    ...
                    {/* Imaging Studies */}
                    <div>
                      <h4 className="text-md font-medium text-gray-900 mb-3">Chẩn đoán hình ảnh</h4>
                      <div className="space-y-3">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
                          ...
                          <textarea
                            value={Array.isArray(getValue("paraclinicalIndications.imagingStudies.studies")) ? getValue("paraclinicalIndications.imagingStudies.studies").join('\n') : ''}
                            onChange={(e) =>
                              updateRecord("paraclinicalIndications.imagingStudies.studies", e.target.value.split('\n').filter(s => s.trim()))
                            }
                            rows={4}
                        ...
                        <div>
                          ...
                          <textarea
                            value={getValue("paraclinicalIndications.imagingStudies.notes")}
                            onChange={(e) =>
                              updateRecord("paraclinicalIndications.imagingStudies.notes", e.target.value)
                            }
                            rows={2}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                    ...
                    {/* Procedures */}
                    <div>
                      <h4 className="text-md font-medium text-gray-900 mb-3">Thủ thuật</h4>
                      <div className="space-y-3">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
                            Danh sách thủ thuật
                          ...
                          <textarea
                            value={Array.isArray(getValue("paraclinicalIndications.procedures.procedures")) ? getValue("paraclinicalIndications.procedures.procedures").join('\n') : ''}
                            onChange={(e) =>
                              updateRecord("paraclinicalIndications.procedures.procedures", e.target.value.split('\n').filter(p => p.trim()))
                            }
                            rows={4}
                        ...
                        <div>
                          ...
                          <textarea
                            value={getValue("paraclinicalIndications.procedures.notes")}
                            onChange={(e) =>
                              updateRecord("paraclinicalIndications.procedures.notes", e.target.value)
                            }
                            rows={2}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          />
                    ...
                    {/* Consultations */}
                    <div>
                      <h4 className="text-md font-medium text-gray-900 mb-3">Hội chẩn</h4>
                      <textarea
                        value={getValue("paraclinicalIndications.consultations")}
                        onChange={(e) =>
                          updateRecord("paraclinicalIndications.consultations", e.target.value)
                        }
                        rows={4}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="text"
                        value={getValue("paraclinicalIndications.resultLocation")}
                        onChange={(e) =>
                          updateRecord("paraclinicalIndications.resultLocation", e.target.value)
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                        <p className="text-sm text-gray-600">Kéo thả file kết quả</p>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                            ...
                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                              {commonIcdCodes
                                .filter(item => 
                                  ...
                                  item.description.toLowerCase().includes(icdSearchTerm.toLowerCase())
                                )
                                .map((item, index) => (
                                  <div
                                    key={index}
                                    ...
                                    onClick={() => {
                                      ...
                                      if (!currentCodes.includes(item.code)) {
                                        updateRecord("icdCodes", [...currentCodes, item.code]);
                                      }
                                      setIcdSearchTerm("");
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(getValue("icdCodes") || []).map((code, index) => (
                            <span
                              key={index}
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const currentCodes = getValue("icdCodes") || [];
                                  updateRecord("icdCodes", currentCodes.filter(c => c !== code));
                                }}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.value.trim()) {
                              const currentCodes = getValue("icdCodes") || [];
                              const newCodes = e.target.value.split(',').map(code => code.trim()).filter(code => code);
                              updateRecord("icdCodes", [...currentCodes, ...newCodes]);
                              e.target.value = "";
                            }
                          }}
                          onKeyPress={(e) => {
                            if (e.key === 'Enter' && e.target.value.trim()) {
                              const currentCodes = getValue("icdCodes") || [];
                              if (!currentCodes.includes(e.target.value.trim())) {
                                updateRecord("icdCodes", [...currentCodes, e.target.value.trim()]);
                              }
                              e.target.value = "";
                            }
                          }}
              ...
              {activeTab === "treatment" && (
                <div className="space-y-6">
                  ...
                  <div>
                   ...
                   {/* Medications */}
                   <div>
                     <h4 className="text-md font-medium text-gray-900 mb-3">Danh sách thuốc</h4>
                     
                     {/* Added Medications List */}
                     {medicationsList.length > 0 && (
                       <div className="mb-6">
                         <h5 className="text-sm font-medium text-gray-700 mb-3">Thuốc đã thêm:</h5>
                         <div className="space-y-3">
                             ...
                             <div key={med.id} className="bg-gray-50 p-4 rounded-lg border">
                               <div className="flex justify-between items-start">
                                 <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3">
                                   <div>
                                     <span className="font-medium text-blue-600">{med.drugName}</span>
                                     <span className="text-gray-600 ml-2">({med.strength})</span>
                                     <div className="text-sm text-gray-500">{med.form}</div>
                                   </div>
                                   <div>
                                     ...
                                     <div className="text-sm">
                                       <span className="font-medium">Tần suất:</span> {med.frequency} lần/ngày × {med.duration} ngày
                                     </div>
                                     <div className="text-sm">
                                       <span className="font-medium">Tổng số:</span> {calculateTotalQuantity(med.frequency, med.duration)} {med.form}
                                     </div>
                                   </div>
                                   <div>
                                     <div className="text-sm">
                                       <span className="font-medium">Hướng dẫn:</span>
                                       <div className="text-gray-600 mt-1">{med.instructions || "Theo chỉ định bác sĩ"}</div>
                                     </div>
                                   </div>
                                 </div>
                                 <button
                     ...
                     <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                       <h5 className="text-sm font-medium text-gray-700 mb-3">Thêm thuốc mới:</h5>
                       <div className="space-y-4">
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                           <div>
                             ...
                             <input
                               ...
                               onChange={(e) =>
                                 updateRecord("medications.drugName", e.target.value)
                               }
                           ...
                           <div>
                             ...
                             <input
                               ...
                               onChange={(e) =>
                                 updateRecord("medications.strength", e.target.value)
                               }
                           ...
                           <div>
                             ...
                             <input
                               ...
                               onChange={(e) =>
                                 updateRecord("medications.dosage", e.target.value)
                               }
                           ...
                           <div>
                             ...
                             <select
                               ...
                               onChange={(e) =>
                                 updateRecord("medications.frequency", parseInt(e.target.value))
                               }
                               className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                             >
                           ...
                           <div>
                             ...
                             <input
                               ...
                               onChange={(e) =>
                                 updateRecord("medications.duration", parseInt(e.target.value))
                               }
                               className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                             />
                           </div>
                         ...
                         {getValue("medications.frequency") && getValue("medications.duration") && (
                           <div className="bg-blue-50 p-3 rounded-lg">
                             <div className="text-sm text-blue-800">
                               <span className="font-medium">Tổng số lượng cần:</span> {calculateTotalQuantity(getValue("medications.frequency"), getValue("medications.duration"))} {getValue("medications.form") || "đơn vị"}
                             </div>
                           </div>
                         )}
                         ...
                         <div>
                           ...
                           <textarea
                             ...
                             onChange={(e) =>
                               updateRecord("medications.instructions", e.target.value)
                             }
                             rows={2}
                   ...
                   <div className="bg-yellow-50 p-4 rounded-lg">
                     <div className="flex items-center justify-between">
                       <div>
                         <h4 className="text-md font-medium text-gray-900">Phát hành đơn thuốc</h4>
                         <p className="text-sm text-gray-600">Tạo Prescription + PDF URL, cập nhật status: "prescription_issued"</p>
                       </div>
                       <button
                         onClick={() => {
                     ...
                     <div className="space-y-3">
                       <div>
                         ...
                         <textarea
                           ...
                           onChange={(e) =>
                             updateRecord("followUpCare.instructions", e.target.value)
                           }
                           rows={4}
                     ...
                     <div className="space-y-3">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                         <div>
                           ...
                           onChange={(e) =>
                             updateRecord("followUpCare.warningSignsEducation", e.target.value)
                           }
                             rows={3}
                         ...
                         <div>
                           ...
                           onChange={(e) =>
                             updateRecord("followUpCare.emergencyContacts", e.target.value)
                           }
                             rows={3}
                       ...
                       <div className="bg-red-100 p-3 rounded-lg">
                         <h5 className="text-sm font-medium text-red-900 mb-2">Liên hệ khẩn cấp:</h5>
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                           <div>
                             <span className="font-medium">Cấp cứu:</span> 115
                           </div>
                           <div>
                             <span className="font-medium">Bác sĩ điều trị:</span> {user?.phone || "Chưa cập nhật"}
                           </div>
                           <div>
                             <span className="font-medium">Phòng khám:</span> 1900-xxxx
                           </div>
                         </div>
                       </div>
                     </div>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div>
                           ...
                           <select
                             ...
                             onChange={(e) =>
                               updateRecord("followUpCare.nextAppointment.notes", e.target.value)
                             }
                         ...
                         <div>
                           ...
                           <input
                             type="date"
                             value={getValue("followUpCare.nextAppointment.date")}
                             onChange={(e) =>
                               updateRecord("followUpCare.nextAppointment.date", e.target.value)
                             }
                             className="w-full px-3 py-2 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white"
                           />
                         </div>
               ...
               {activeTab === "documents" && (
                 <div className="space-y-6">
                   ...
                   {/* Attachments */}
                   <div>
                     <h4 className="text-md font-medium text-gray-900 mb-3">Tệp đính kèm</h4>
                     <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                       <Camera className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                       <p className="text-sm text-gray-600 mb-2">Kéo thả hoặc click để tải lên ảnh/clip/biểu mẫu</p>
                       <input
                         type="file"
                         multiple
                     ...
                     <div className="mt-4">
                       <h5 className="text-sm font-medium text-gray-700 mb-2">Tệp đã tải lên:</h5>
                       <div className="space-y-2">
                         {Array.isArray(getValue("attachments")) ? getValue("attachments").map((file: any, index: number) => (
                         ...
                         )) : <p className="text-sm text-gray-500">Chưa có tệp nào được tải lên</p>}
                       </div>
                     </div>
                   </div>
                   ...
                   {/* Audit Log */}
                   <div>
                     <h4 className="text-md font-medium text-gray-900 mb-3">Nhật ký & kiểm soát</h4>
                     <div className="bg-gray-50 p-4 rounded-lg">
                       ...
                       <div>
                         <h5 className="text-sm font-medium text-gray-700 mb-2">Lịch sử chỉnh sửa:</h5>
                         <div className="space-y-1 max-h-32 overflow-y-auto">
                           {Array.isArray(getValue("audit")) ? getValue("audit").map((entry: any, index: number) => (
                             <div key={index} className="text-xs text-gray-600 p-2 bg-white rounded">
                               <span className="font-medium">{entry.user}</span> - {entry.action} - {new Date(entry.timestamp).toLocaleString()}
                             </div>
                           )) : <p className="text-xs text-gray-500">Chưa có lịch sử chỉnh sửa</p>}
                         </div>
                       </div>
                     </div>
                   </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  ...
                  record.status === 'prescription_issued' ? 'bg-blue-100 text-blue-800' :
                  record.status === 'draft' ? 'bg-yellow-100 text-yellow-800' :
                  'bg-gray-100 text-gray-800'
                }`}>
                  {record.status === 'completed' ? 'Hoàn thành' :
                   record.status === 'prescription_issued' ? 'Đã phát hành đơn' :
                   record.status === 'draft' ? 'Nháp' : 'Đang xử lý'}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  disabled={saving || !record || record.status === 'completed' || !record.treatment?.medicationsList?.length}
