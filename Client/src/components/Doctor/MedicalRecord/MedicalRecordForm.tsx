...
const MedicalRecordForm: React.FC<MedicalRecordFormProps> = ({
  ...
  const [autoFilledInfo, setAutoFilledInfo] = useState<string[]>([]);
  const [showAutoFillNotification, setShowAutoFillNotification] =
    useState(false);

  // Use refs to track loading states without causing dependency issues
  ...
  const loadMedicalRecord = useCallback(async () => {
    // Prevent multiple simultaneous record creation
    if (loadingRef.current || isCreatingRecordRef.current) {
      console.log("Already loading or creating record, skipping...");
      return;
    }
    ...
    setError("");
    try {
      console.log("Loading medical record for appointment:", appointmentId);
      ...
      if (data && data._id) {
        console.log("Found existing medical record:", data._id);
        setRecord(data);
        ...
        try {
          ...
          console.log("Created new medical record:", newRecord._id);
          setRecord(newRecord);
          setMedicationsList([]); // Reset medications list for new record

          // Check for auto-filled information
          const autoFilled = [];
          if (newRecord.medicalHistory?.pastMedicalHistory) {
            autoFilled.push("Bệnh sử");
          }
          if (newRecord.medicalHistory?.surgicalHistory) {
            autoFilled.push("Tiền sử phẫu thuật");
          }
          if (newRecord.medicalHistory?.familyHistory) {
            autoFilled.push("Tiền sử gia đình");
          }
          if (newRecord.medicalHistory?.socialHistory) {
            autoFilled.push("Tiền sử xã hội");
          }
          if (newRecord.quickScreening?.allergies?.allergyList) {
            autoFilled.push("Dị ứng");
          }
          if (newRecord.quickScreening?.currentMedications) {
            autoFilled.push("Thuốc hiện tại");
          }
          if (
            newRecord.quickScreening?.pregnancyStatus?.isPregnant !== undefined
          ) {
            autoFilled.push("Thông tin thai sản");
          }

          if (autoFilled.length > 0) {
            setAutoFilledInfo(autoFilled);
      ...
      isCreatingRecordRef.current = false; // Reset flag on error
    } finally {
      loadingRef.current = false;
      setLoading(false);
    }
  }, [appointmentId, user?._id]); // Using refs for loading states to avoid dependency issues

  // Load medical record when modal opens - with debouncing
  useEffect(() => {
  ...
  const handleSave = async () => {
    if (!record?._id || !user?._id || saving) return;
    ...
    try {
      ...
      const recordToSave = {
        ...
        prescription: {
          ...record.prescription,
          notes: record.prescription?.notes || "", // Ensure notes is always provided
      ...
      console.log("Saving medical record:", record._id);
      ...
      <Alert severity="success">Lưu thành công bệnh án.</Alert>;
    } catch (err) {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      console.log("Saving draft with data:", {
        recordId: record._id,
        hasFinaldDiagnosis: !!draftRecord.finalDiagnosis,
        finalDiagnosis: draftRecord.finalDiagnosis,
        hasPreliminaryDiagnosis: !!draftRecord.preliminaryDiagnosis,
        preliminaryDiagnosis: draftRecord.preliminaryDiagnosis,
      ...
      setRecord(updatedRecord);
      onSave?.(updatedRecord);
      <Alert severity="warning">Lưu nháp bệnh án thành công</Alert>;
    } catch (err) {
...
Bác sĩ: ${user?.name || "N/A"}
  ...
  const handleCompleteRecord = async () => {
    ...
    // Check required fields according to MedicalRecord model
    ...
    if (!record.preliminaryDiagnosis) {
  ...
  const updateRecord = React.useCallback(
    ...
    [] // Không phụ thuộc vào record, sử dụng functional update
  );
  ...
  const getNumberValue = (path: string): number => {
  ...
  // Loại bỏ useEffect tự động calculate BMI để tránh vòng lặp vô hạn
  // BMI sẽ được tính khi user thay đổi weight/height
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center bg-black/60 justify-center p-4 bg-opacity-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-7xl max-h-[95vh] overflow-hidden">
        ...
        {showAutoFillNotification && autoFilledInfo.length > 0 && (
          <div className="mx-6 mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-start gap-3">
              <CheckCircle className="h-5 w-5 text-green-600 mt-0.5" />
              <div className="flex-1">
                <h4 className="text-sm font-medium text-green-900 mb-1">
                  Thông tin đã được tự động điền từ lần khám trước
                </h4>
                <p className="text-sm text-green-700 mb-2">
                  Các thông tin sau đã được tự động điền và có thể chỉnh sửa nếu
                  cần:
                </p>
                <div className="flex flex-wrap gap-1">
                  {autoFilledInfo.map((info, index) => (
                    <span
                      key={index}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // Tính lại BMI khi weight thay đổi
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // Tính lại BMI khi height thay đổi
                            setTimeout(() => calculateBMI(), 100);
                          }}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
      ...
      {showHistoryPanel && record?.patient && user?._id && (
        <PatientHistoryPanel
          patientId={
            typeof record.patient === "string"
              ...
              : typeof record.patient === "object" &&
                record.patient !== null &&
                "_id" in record.patient
              ? (record.patient as { _id: string })._id
              : ""
          }
          doctorId={user._id}
