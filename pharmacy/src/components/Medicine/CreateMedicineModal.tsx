...
const CreateMedicineModal: React.FC<CreateMedicineModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Require token to submit
    if (!token) {
      setError("Vui lòng đăng nhập bằng tài khoản staff để thêm thuốc");
      return;
    }
    ...
    try {
      // Clean up form data
      const cleanedData: MedicineFormData = {
        ...formData,
      ...
      await medicineApi.createMedicine(cleanedData);

      onSuccess();
      onClose();
      onNotify?.("Tạo thuốc thành công", "success");
      // Reset form
      setFormData({
        name: "",
        genericName: "",
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Có lỗi xảy ra khi tạo thuốc";
    ...
    <div className="fixed inset-0 bg-black/80 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-900">Thêm thuốc mới</h2>
          <button
            onClick={onClose}
