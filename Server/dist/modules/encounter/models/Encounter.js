...
const EncounterSchema = new mongoose_1.Schema({
    ...
    vitals: mongoose_1.Schema.Types.Mixed,
    prescriptions: [mongoose_1.Schema.Types.Mixed],
    summary: String,
}, { timestamps: true });
// Generate encounter number
EncounterSchema.pre("validate", function (next) {
    return __awaiter(this, void 0, void 0, function* () {
        if (this.isNew && !this.encounterNumber) {
            try {
                const count = yield mongoose_1.default.model("Encounter").countDocuments();
                this.encounterNumber = `ENC-${Date.now()}-${String(count + 1).padStart(4, "0")}`;
            }
            catch (err) {
                return next(err);
            }
