...
export const forgotPassword = async (
  ...
  try {
    ...
    patient.resetPasswordExpires = expires as unknown as Date;
    await patient.save();
                ...
                <div style="display: flex; align-items: center; margin-bottom: 15px;">
                    ...
                    <div>
                        <p style="color: #1e293b; margin: 0; font-size: 18px; font-weight: 600;">
                            Xin chào, <span style="background: linear-gradient(135deg, #3b82f6, #14b8a6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 700;">${
                              patient.name || "bạn"
                            }</span>!
                        </p>
                    </div>
    ...
    res.status(200).json({
      success: true,
      message:
        "Liên kết đặt lại mật khẩu đã được tạo. Vui lòng kiểm tra email (nếu không nhận được, dùng resetLink bên dưới).",
      token, // dev only
...
export const resetPassword = async (
  ...
  try {
    ...
    patient.resetPasswordExpires = undefined as unknown as Date;
    await patient.save();
